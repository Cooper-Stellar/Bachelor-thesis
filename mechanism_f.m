function EP = mechanism(road,user,SAV)
    iteration = 5000; % 反復回数
...
%     %%}
    ...
    %A_S = sparse(A);
    b = sparse([ones(1,L*(T-1)).*reshape(limit,1,[]) zeros(1,L*(T-1))]);
    ...
    while s<iteration
        ...
        %A_S = sparse(A);
        b = sparse([ones(1,L*(T-1)).*reshape(limit,1,[])]);
        Aeq = [
        ...
        y = (s/(s+1))*y+(1/(s+1))*y_new;
        h = (s/(s+1))*h+(1/(s+1))*h_new;
        ...
        x = (s/(s+1))*x+(1/(s+1))*x_new;
        f = (s/(s+1))*f+(1/(s+1))*f_new;
    ...
    %%{
    figure % p(5,9)
    p_5_9_rec = zeros(1,iteration);
    for i=1:iteration
        p_5_9_rec(1,i) = p_rec(5,9,i);
    end
    plot(p_5_9_rec,'LineWidth',1.0); hold on
    yline(EP_p(5,9),'--r','LineWidth',0.8); hold off
    xlim([-0.1*s 1.1*s])
    grid on
    title('ij=5,t=9のサービス価格の推移')
    xlabel('Day'); ylabel('価格')
    lgd = legend({'MS価格','均衡MS価格'},'FontSize',14,'TextColor','black','Location','southeast');
    %%}
    ...
    figure %経路全体のMS価格の推移
    plot(p_1_7_rec+p_5_9_rec,'LineWidth',0.8); hold on
    yline(EP_p(1,7)+EP_p(5,9),'--r'); hold off
    xlim([-0.1*s 1.1*s])
    grid on
    title('経路全体のMS価格の推移')
    xlabel('Day'); ylabel('価格')
    ...
    figure
    plot(OD_each(1,:),'LineWidth',0.5); hold on
    for i=2:SP_U_X_num
        plot(OD_each(i,:),'LineWidth',0.5);
    end
    %yline(EP_SPU,'--r','LineWidth',0.8);
    hold off
    xlim([-0.1*s 1.1*s])
    grid on
